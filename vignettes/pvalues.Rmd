...
Compare mean `len` depending on `supp`. Error bars indicate 95% confidence 
intervals.
...
# normal plot
p + add_pvalue(df_p_val, y.position = 28)
```
...
`26` in this case. Error bars indicate 95% confidence intervals.
...
```{r, fig.height=7}
# add our grouping variable to ToothGrowth
tg <- ToothGrowth
